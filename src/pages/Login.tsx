...
import { saveAccessToken } from "../libs/axios";
  ...
  const handleSuccess = async (codeResponse: CodeResponse) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || `Backend error: ${res.statusText}`);
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg">
        ...
        <div className="mt-8 space-y-6">
          <button
            onClick={() => googleLogin()}
            className="w-full flex justify-center items-center px-4 py-3 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200"
          >
