...
import { sendEmail } from "../services/email-services"; // Import the service
...
const ComposeModal: React.FC = () => {
    ...
    const handleSend = async () => {
        ...
        try {
            const payload = {
                ...
                bcc,
                subject,
                bodyHtml: body, // Map 'body' state to 'bodyHtml' DTO field
                replyToEmailId: composeData.replyToId // If you add this to Context later
            };
            ...
            // Optional: You might want to trigger a toast notification here
    ...
    if (minimized) {
        return (
            <div
                className="fixed bottom-0 right-20 w-64 z-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-t-lg shadow-xl cursor-pointer"
    ...
    return (
        <div className="fixed bottom-0 right-20 w-[800px] h-[700px] z-50 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-t-lg shadow-2xl flex flex-col font-sans">
            {/* Header */}
            <div
                className="flex items-center justify-between px-4 py-2 bg-gray-100 dark:bg-gray-900 rounded-t-lg border-b dark:border-gray-700 cursor-pointer"
                onClick={toggleMinimize}
            >
                ...
                <div className="flex items-center space-x-3 text-gray-500">
                    <button onClick={(e) => { e.stopPropagation(); toggleMinimize(); }} className="hover:text-gray-800 dark:hover:text-white">
                        <span className="material-icons-outlined text-lg">minimize</span>
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); closeCompose(); }} className="hover:text-gray-800 dark:hover:text-white">
            ...
            <div className="px-4 py-2 space-y-1 border-b dark:border-gray-700 bg-white dark:bg-gray-800">
                ...
                <input
                    className="w-full border-none focus:ring-0 outline-none bg-transparent text-sm py-2 font-medium dark:text-white placeholder-gray-500"
                    placeholder="Subject"
                    value={subject}
            ...
            <div className="p-3 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                <div className="flex items-center space-x-2">
                    <button
                        onClick={handleSend}
                        disabled={isSending}
                        className={`px-5 py-2 rounded-full text-sm font-bold shadow-sm transition-colors text-white 
                            ${isSending ? 'bg-blue-400 cursor-not-allowed' : 'bg-primary hover:bg-blue-700'}`}
                    >
